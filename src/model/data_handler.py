...
def generate_data(path, b=2, d=20):
    ...
    heuristics = torch.tensor(heuristics).unsqueeze(1)
...
def gen_dataset(num_data, batch_size):
    '''
    Args:
...
def load_dataset(filepath):
    s = time.time()
    (states, heuristics) = torch.load(filepath)
    print("Dataset loaded in {:.2}s".format(time.time() - s))
    return 
    print(states.shape, heuristics.shape)
